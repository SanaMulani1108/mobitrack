...
public class sell_mobile extends JFrame implements ActionListener,TextListener
{
	...
	sell_mobile()
	{
		...
		p1.add(l4);p1.add(t4);p1.add(l5);p1.add(c);
	...
	public void actionPerformed(ActionEvent ae)
	{
		...
		if(s.equals("INSERT"))
		{
			try
			{
				...
				PreparedStatement ps=co.prepareStatement("insert into sale value(?,?,?,?,?,?,?,?,?,?)");
				ps.setInt(1,a);
				ps.setString(2,s1);
				ps.setString(3,s2);
				ps.setString(4,b);
		...
		else if(s.equals("UPDATE"))
		{
			try
			{
				...
				PreparedStatement ps=co.prepareStatement("update sale set cust_nm=?,cust_mob=?,date=?,prod_nm=?,comp_nm=?,prod_price=?,quantity=?,warranty=?,Total_Amount=? where Prod_ID="+a);
		...
		else if(s.equals("DELETE"))
		{
			try
			{
				...
				PreparedStatement ps=co.prepareStatement("delete from sale where Prod_ID="+a);
			...
			ll6=new JLabel("Date                   :"+t4.getText());
			 ...
			 ll11=new JLabel("Prod Name");
			  ll16=new JLabel(""+t9.getText());
				...
				ll17=new JLabel(""+c.getSelectedItem());
